...
export function TaskDetails() {
    ...
    return (
        <section className={`task-details ${animationClass}`}>
            {task ? (
                <>
                    <button className="exit-btn" onClick={closeTaskDetails}>{svgs.exit}</button>

                    <section className="details-top-bar">
                        <h2>{task.title}</h2>
                        <section className="task-details-actions">
                            <button className={view === 'updates' ? 'active' : ''} onClick={() => setView('updates')}><span>{svgs.home}</span>&nbsp;Updates</button>
