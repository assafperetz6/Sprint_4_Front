...
export function CollapsedGroupPreview({ group, cmpsOrder, idx, dragHandleProps }) {
  return (
    <>
      <section className="sticky-container" {...dragHandleProps}>
...
export function CollapsedDraggedPreview({ group, idx }) {
  ...
  return (
    <Draggable draggableId={group.id} index={idx}>
      {(provided) => (
        <section className="sticky-container" ref={provided.innerRef} {...provided.dragHandleProps}>
          {/* <div className="white-space"></div> */}
          <div className="colored-border" style={{ backgroundColor: hexToRgba(group.style.color, 1) }}></div>
