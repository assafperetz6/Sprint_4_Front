...
=======
				{groupSummaryArray.map((summary, idx) => (
					<DynamicSummaryCmp padding={8} summary={summary} statusLabels={statusLabels} priorityLabels={priorityLabels} key={idx} />
>>>>>>> f-dnd-copy
				))}
			</section>
		</section>
	...
	return (
		<section style={{ width: width + 'px' }} className="status-summary summary">
			{summaryForRender.map(summary => (
				<div className="summary-colored-block" key={makeId()} style={{ backgroundColor: summary.color, width: unit * summary.size + 'px' }}></div>
			))}
		</section>
	)
}
