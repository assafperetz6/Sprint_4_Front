...
export function TimelinePicker({ task, group, defaultWidth }) {
  ...
  function handleRangeSelect(range) {
    ...
    try {
      ...
      const activity = {
        oldState: task.timeline || null,
        newState: timelineToSave,
        type: 'Timeline'
      }
    ...
    <li className="timeline-picker" style={{ width: defaultWidth }}>
      <div ref={buttonRef} className="timeline-bar" style={getProgressStyle()} onClick={handleOpenModal}>
        <span
          ...
          data-content={`${task.timeline ? formatRange(selectedRange) : '-'}`}
          data-hover-content={selectedRange.startDate && selectedRange.endDate ? getDaysBetween() : 'Set Dates'}></span>
      </div>
