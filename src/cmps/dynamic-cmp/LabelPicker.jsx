...
export function LabelPicker({ cmp, task, groupId, defaultWidth }) {
  ...
  async function onChangeLabel(ev, newLabel) {
    ...
    try {
      ...
      const activity = {
        oldState: task[labelTaskName],
        newState: newLabel.id,
        type: labelTaskName === 'status' ? 'Status' : 'Priority'
      }

      updateTask(board._id, groupId, taskToSave, activity)
      setLabel(newLabel)
    } catch (error) {
    ...
    <div className="label-picker-popup" ref={popperRef} style={styles.popper} {...attributes.popper}>
      ...
      <ul className="labels-list clean-list">
        {board[labelsName].map((label) => (
          <li
            key={label.id}
            style={{ backgroundColor: label.color, gridTemplateRows: board[labelsName].length }}
